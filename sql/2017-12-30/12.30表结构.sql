...
) ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARSET=utf8 COMMENT='黑名单表';
...
) ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARSET=utf8 COMMENT='违章记录表';
...
) ENGINE=InnoDB AUTO_INCREMENT=239 DEFAULT CHARSET=utf8 COMMENT='Ա�����Ա�';
...
CREATE TABLE `employeeoutdistribute` (
  ...
  `unitid` varchar(4) DEFAULT NULL COMMENT '单位ID',
...
) ENGINE=InnoDB AUTO_INCREMENT=22 DEFAULT CHARSET=utf8 COMMENT='违章记录表';
...
) ENGINE=InnoDB AUTO_INCREMENT=2036 DEFAULT CHARSET=utf8 COMMENT='��ʷ�Ծ�ѡ���';
...
/* Event structure for event `event_clearEmployOutBlack` */

DELIMITER $$

/*!50106 CREATE DEFINER=`root`@`localhost` EVENT `event_clearEmployOutBlack` ON SCHEDULE EVERY 1 MINUTE STARTS '2017-12-06 11:49:37' ON COMPLETION NOT PRESERVE ENABLE DO CALL clearEmploutBlack() */$$
DELIMITER ;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `overhaul_unit` AS select `haulinfo`.`bigId` AS `id`,(case when 1 then NULL else 1 end) AS `upid`,`haulinfo`.`bigName` AS `name`,`haulinfo`.`bigCreateDate` AS `bigCreateDate` from `haulinfo` union select `haulunit`.`unitId` AS `id`,`haulunit`.`bigId` AS `upid`,(select `unit`.`name` from `unit` where (`unit`.`unitId` = `haulunit`.`unitId`)) AS `name`,(select `haulinfo`.`bigCreateDate` from `haulinfo` where (`haulinfo`.`bigId` = `haulunit`.`bigId`)) AS `bigCreateDate` from `haulunit` order by `bigCreateDate` desc */;
