...
/*		if(listInfo.size()==1){			
		...
		if (examIds != null && examIds.size() > 0) {		
			//删除成绩表中的信息,根据考试ID集合和员工身份证号删除成绩信息
			...
			criteriaEmployeeExamInfo.andExamidIn(examIds);
			criteriaEmployeeExamInfo.andEmployeeidEqualTo(employeeOutBaseInfo.getIdcard());
