...
$(function () {
    ...
    $(".el_addquestions").click(function () {
        ...
        if (index == "单选题") {
            ...
            qContent.children(".danxuan").load("danxuanOptions.txt",function(response){
            ...
            qContent.children(".panduan").empty();
            $(".el_bigBlock2").css("height", "300px");        
            qContent.children(".duoxuan").load("duoxuanOptions.txt",function(response){
            ...
            qContent.children(".panduan").load("panduan.txt",function(response){
...
function queryQuestionBankNameByDeptId(){
	$.ajax({
		...
		type :"POST",
		data:{"departmentId":departmentIdFromSess},
		dataType:"json",
		async:true,
		success:function(data){
			...
			for(var i=0;i<allQuestionBankName.length;i++){
				if(allQuestionBankName[i].questionBankId == questionBankId_Old)
					//设置默认选中第一条  value值设置题库ID，标签中间设置题库名称
					optionStrQuestionBankName += "<option value='" + allQuestionBankName[i].questionBankId+"' selected>"+allQuestionBankName[i].questionBankName+"</option>";
				else	
					optionStrQuestionBankName += "<option value="+allQuestionBankName[i].questionBankId+">"+allQuestionBankName[i].questionBankName+"</option>";
			}
			//先将下拉列表清空
			$("#modify_QuestionBankNameList").empty();
			$("#modify_QuestionBankNameList").append(optionStrQuestionBankName);			
		}
...
function showKnowledgeDic(response) {
	if (response != null && response.names != null) {
		...
		for (var i = 0; i < names.length; i++) {
			if(names[i]==knowledge_old){
				$("#modify_knowledgetypeList").append(
						'<option value="' + names[i] + '" selected>' + names[i]
						+ '</option>')
			}else{				
				$("#modify_knowledgetypeList").append(
...
function modify_question(){
	...
	var isNotNull = $("#form_modifyQuestion").validate({
		rules:{
		...
		messages:{
			"question.questionbankid":{required:"请先创建一个题库！"},
			"question_answer" :{required : "至少选择两个答案"}
		},
	...
	if(isNotNull.form()){
		...
		for(var i=0;i<optionListObj.length;i++){
			if(optionListObj.eq(i).val().length>0){			
				var j = i+1;
				optionListInfo +="<input name='options["+i+"].optioncontent' type='hidden' value='"+optionListObj.eq(i).val()+"'/>"
				+"<input name='options["+i+"].optionsequence' type='hidden' value='"+j+"'/>";
				optionsIsNotNull = true;
			}else{
				alert("第"+(i+1)+"个选项不能为空！");
		...
		if(optionsIsNotNull){
			...
			if(answer.length>0){		
				//拼接试题答案
				optionListInfo += "<input name='question.answer' type='hidden' value='"+answer+"'/>";
			}
			$("#updateOptions_content").append(optionListInfo);
			
			$.ajax({
				url : 'questions_modifyQuestionInfo.action',
				data : $("#form_modifyQuestion").serialize(),
				type : 'POST',
				dataType : 'json',
				async:true,
				success : function(data) {
