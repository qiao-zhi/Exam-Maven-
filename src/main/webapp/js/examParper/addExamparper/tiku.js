...
$(function() {
	...
	$("#hide_tiku").click(function() {
		$(".el_addEPtiku").animate({
			right : '-37%'
		}, 800);
		$(".el_addEPleft").stop().animate({
			width : '100%'
		}, 600);
		$("#show_tiku").show();
	})
...
function el_tiku_checkedButtonF(obj) {
	...
	/*
	 * // 获取试题解析 var ques_analy = $(obj).parent().find(".ques_analy").val();
	 */
	// 获取答案
	...
	if ($(obj).is(":checked")) { // 选中执行
		...
		if (ques.hasClass("dan")) {// 单选
			...
			var quesLeft = '<div class="movie_box">'
					+ '<div class="tm_btitlt"><i class="nmb">' + ques_num
					+ '</i>. <i class="btwenzi">' + ques_title + '</i></div>'
					+ '<ul class="wjdc_list">';// 试题编号和题干
			var quesMiddle = "";
			ques_options.each(function() {
				quesMiddle = quesMiddle + '<li><label><input name="a'
						+ ques_num + '" type="radio"><span>' + $(this).text()
						+ '</span></label></li>';
			});// 选项
			var quesRight = '</ul>'
					...
					+ ques_answer
					+ '">'
					+ updateAnswerFormat(ques_answer, "12345", "ABCDE")
					+ '<br/>'
					+
					/*
					 * '解析:<input type="hidden" class="ques_analy"
					 * value="'+ques_analy+'">' + ques_analy;
					 */'</div>';
			// 3、生成试题，把试题放到左边区域中
			...
			var everyQuesScore = $(".danxuanBigTitle")
					.find(".el_modifiedGrade").val()// 获取每道题分值
			$(".danxuanBigTitle").find(".numTotal").text(
					parseFloat(everyQuesScore) * parseFloat(ques_length)
							+ "分;共")// 设置分总数
		...
		if (ques.hasClass("duo")) {// 多选
			...
			var quesMiddle = "";
			ques_options.each(function() {
				quesMiddle = quesMiddle + '<li><label><input name="b'
						+ ques_num + '" type="checkbox"><span>'
						+ $(this).text() + '</span></label></li>';
			});
			var quesRight = '</ul>'
					+ "<input type='hidden' id='"
					+ ques_id
			...
			var everyQuesScore = $(".duoxuanBigTitle")
					.find(".el_modifiedGrade").val()// 获取每道题分值
		...
		if (ques.hasClass("pan")) {// 判断
			...
			var quesFull = '<div class="movie_box">'
					+ '<div class="tm_btitlt"><i class="nmb">' + ques_num
					+ '</i>. <i class="btwenzi">' + ques_title + '</i></div>'
					+ '<ul class="wjdc_list">' + '<li><label><input name="c'
					+ ques_num + '" type="radio"><span>正确</span></label></li>'
					+ '<li><label><input name="c' + ques_num
					+ '" type="radio"><span>错误</span></label></li>' + '</ul>'
			...
			var everyQuesScore = $(".panduanBigTitle")
					.find(".el_modifiedGrade").val()// 获取每道题分值
		...
		if (ques.hasClass("dan")) {// 单选
			var cancelQues = $(".el_danxuan").children(".movie_box").children(
					".ques_id");
			cancelQues.each(function() {
				if ($(this).prop("id") == ques_id) {
		...
		if (ques.hasClass("duo")) {// 多选
			var cancelQues = $(".el_duoxuan").children(".movie_box").children(
					".ques_id");
			cancelQues.each(function() {
				if ($(this).prop("id") == ques_id) {
		...
		if (ques.hasClass("pan")) {// 判断
			var cancelQues = $(".el_panduan").children(".movie_box").children(
					".ques_id");
			cancelQues.each(function() {
				if ($(this).prop("id") == ques_id) {
