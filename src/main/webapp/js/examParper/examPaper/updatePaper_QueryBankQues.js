...
function validateCheckedQuestion() {
	var $leftIds = $(".el_addEPleft").find(".btwenzi")// 查到左边隐藏的ID元素
	var leftIds = [];// 获取到左边已经选择题的题目
	for (var i = 0, length_1 = $leftIds.length; i < length_1; i++) {
		leftIds[i] = $($leftIds[i]).text();
	}
	var $rightIds = $(".el_addEPtiku").find(".tm_btitlt")// 查到右边题的题目
	// 根据左边的元素与右边查找如果已经个选中的话打上对勾
	for (var k = 0, length_2 = leftIds.length; k < length_2; k++) {
		for (var lx = 0, length_3 = $rightIds.length; lx < length_3; lx++) {
			if (leftIds[k] == ($($rightIds[lx]).text())) {
				$($rightIds[lx]).parent().siblings(".el_tiku_checkedButton").attr("checked", "ehecked");
				break;
			}
		}
	}
