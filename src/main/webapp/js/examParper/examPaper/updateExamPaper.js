...
var geneBigForm=function(){
    ...
    for(var i = 0; i < danxuan_length; i ++) {
        ...
        // 解析
        /*
		 * var dan_ques_right = '<input type="hidden"
		 * name="danxuanDati.questions['+ i +'].analysis" value="'+
		 * dan_ques_analy + '">';
		 */
    ...
    for(var i = 0; i < duoxuan_length; i ++) {
       ...
       /*
        ...
        // 解析
       /*
		 * var duo_ques_right = '<input type="hidden"
		 * name="duoxuanDati.questions['+ i +'].analysis" value="'+
		 * duo_ques_analy + '">';
		 */
    ...
    /*
	 * // 解析 var pan_ques_right = '<input type="hidden"
	 * name="panduanDati.questions['+ i +'].analysis" value="'+ pan_ques_analy +
	 * '">';
	 */
 ...
 savePaper();
	...
	function savePaper(){
		// 进行数据验证
		var isNotNull = $("#saveForm").validate({
			ignore : [],
			rules : {
...
/*				"exampaper.description" : {
		...
		$("#el_saveModify").prop("disabled", true);
			$.ajax({
				url:contextPath+'/update_updatePaper.action',
				async:true,
				type:'post',
				dataType:'json',
...
/** ****S 初始化知识点字典********* */
+function initKnowDic() {
	$.post(contextPath + '/dic_getDicNamesByUpid.action', {
		"upId" : "200"
	}, showKnowDic, 'json');
}();
function showKnowDic(response) {
